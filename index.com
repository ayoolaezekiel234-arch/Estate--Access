...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    .btn-amber { background: #d97706; color: #020617; }
    .btn-rose { background: #be123c; color: #fff; }
...
<body>
  ...
  <div class="container">
    ...
    <!-- Header & Security Hotline -->
    <div class="card">
      <div class="header-row">
        <img src="nitel-logo.png" alt="NITEL Crest" class="logo-badge" onerror="this.style.display='none'; document.getElementById('logo-fallback').style.display='flex';" />
    ...
    <div id="view-resident" class="card">
      ...
      <div class="passcode-box">
        <label style="color: #93c5fd;">🔑 Resident Passcode</label>
        <input id="res-passcode-auth" type="text" value="NITEL2026" />
      </div>
      ...
      <button type="button" class="btn-action btn-blue" onclick="handleGeneratePass()">
    ...
    <div id="view-guard" class="card" style="display: none;">
      <div id="guard-auth-box" style="text-align: center;">
        ...
        <div style="font-size: 11px; color: #94a3b8;">Enter Guard PIN (NCDA / 6232)</div>
      ...
      <div id="guard-terminal-box" style="display: none;">
        ...
        <div style="margin-top: 14px; border-top: 1px solid #334155; padding-top: 10px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
            <span style="font-size:11px; font-weight:bold; color:#cbd5e1; text-transform:uppercase;">Recent Gate Passes</span>
            <button type="button" onclick="loadRecentLogs()" style="background:none; border:none; color:#60a5fa; font-size:10px; font-weight:bold;">🔄 Refresh</button>
          </div>
          <div id="recent-logs-list" style="max-height: 140px; overflow-y: auto; font-size: 11px; display: flex; flex-direction: column; gap: 4px;">
            <p style="color: #64748b; font-style: italic;">No logs loaded.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="view-pass" class="card" style="display: none; text-align: center;">
      ...
      <button id="exit-estate-btn" type="button" class="btn-action btn-rose" onclick="checkoutVisitor()">🚪 I Have Left Estate</button>
  ...
  <script>
    ...
    const RESIDENT_PASSCODE = "NITEL2026";
    ...
    async function handleGeneratePass() {
      var passKey = (document.getElementById("res-passcode-auth").value || "").trim().toUpperCase();
      var host = (document.getElementById("res-host-name").value || "").trim();
      var phone = (document.getElementById("res-host-phone").value || "").trim();
      var zone = document.getElementById("res-zone-select").value;
      var house = (document.getElementById("res-house-no").value || "").trim();
      var visitor = (document.getElementById("res-visitor-name").value || "").trim();
      var type = document.getElementById("res-pass-type").value;

      if (passKey !== RESIDENT_PASSCODE) {
        alert("Incorrect Passcode. Use NITEL2026");
        return;
      }
      if (!visitor) {
      ...
      if (supabaseClient) {
        try {
          await supabaseClient.from('visitor_passes').insert([{
            ...
            access_code: code,
            pass_type: type,
            resident_name: host || 'Resident',
            house_number: zone + (house ? ' - ' + house : '') + (phone ? ' [' + phone + ']' : ''),
            status: 'ACTIVE'
          }]);
        } catch(err) {
          console.warn("Database sync:", err);
        }
      }
    }
    ...
    async function verifyGatePass() {
      ...
      if (!supabaseClient) {
        res.innerHTML = "Database connecting...";
        return;
      }
      ...
      try {
        ...
        if (error || !data) {
          ...
          res.style.background = "#881337";
          res.style.color = "#fda4af";
          res.innerHTML = "<b>ACCESS DENIED</b><br><small>Pass Checked Out</small>";
        } else {
          if (data.pass_type !== 'HOTEL') {
          ...
          res.innerHTML = "<b>ACCESS GRANTED</b><br><small>" + data.visitor_name + " — " + (data.resident_name || 'Resident') + " (" + (data.house_number || 'Plot') + ")</small>";
          loadRecentLogs();
        }
      } catch (err) {
        res.style.background = "#450a0a";
    ...
    async function loadRecentLogs() {
      var list = document.getElementById("recent-logs-list");
      ...
      try {
        ...
        if (!data || data.length === 0) {
          list.innerHTML = "<p style='color:#64748b; font-style:italic;'>No passes created yet.</p>";
          return;
        }

        list.innerHTML = data.map(function(item) {
          var isUsed = item.status === 'USED';
          return "<div style='background:#020617; border:1px solid #1e293b; padding:6px 8px; border-radius:8px; display:flex; justify-content:space-between; align-items:center;'>" +
            "<div><b style='color:#fff; font-size:11px;'>" + item.visitor_name + "</b><br><span style='color:#94a3b8; font-size:10px;'>" + (item.resident_name || 'Resident') + " • " + (item.house_number || 'Plot') + "</span></div>" +
            "<span style='padding:2px 6px; border-radius:4px; font-size:9px; font-weight:bold; " + (isUsed ? "background:#022c22; color:#34d399; border:1px solid #059669;" : "background:#1e3a8a; color:#93c5fd;") + "'>" + item.status + "</span>" +
          "</div>";
        }).join('');
      } catch (e) {
        list.innerHTML = "<p style='color:#64748b; font-style:italic;'>Failed to load entries.</p>";
      }
    }
    ...
    async function checkoutVisitor() {
      ...
      if (currentPassCode && supabaseClient) {
        await supabaseClient.from('visitor_passes').update({ status: 'EXITED' }).eq('access_code', currentPassCode);
      }
